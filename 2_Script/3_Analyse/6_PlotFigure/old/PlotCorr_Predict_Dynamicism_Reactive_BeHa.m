function PlotCorr_Predict_Dynamicism_Reactive_BeHa(Predict_task_All,Predict_reactive_All,Dynamic_task_All,Cluster_BeHa, color_Point,resultsSave)
...
%选择拟合方式：线性拟合（1次多项式）
p = polyfit(Predict_task_All, Cluster_BeHa, 1); % 1表示线性拟合
...
%选择拟合方式：线性拟合（1次多项式）
p = polyfit(Dynamic_task_All, Cluster_BeHa, 1); % 1表示线性拟合

% 生成拟合曲线的 x 和 y 值
x_fit = linspace(min(Dynamic_task_All), max(Dynamic_task_All), 100); % 生成更密集的 x 值用于绘制曲线
y_fit = polyval(p, x_fit); % 计算对应的 y 值

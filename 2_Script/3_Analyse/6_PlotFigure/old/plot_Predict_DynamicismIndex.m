function plot_Predict_DynamicismIndex(Predict_task_All,Dynamic_task_All,Predict_reactive_All,color_Point,resultsSave)
...
saveas(fig1,fullfile(resultsSave,['Predict_Dynamic_OneDay_RT100_500','.svg']))
close(fig1)
...
saveas(fig1,fullfile(resultsSave,['Predict_Reactive_OneDay_RT100_500','.svg']))
close(fig1)
...
saveas(fig1,fullfile(resultsSave,['Reactive_Dynamic_OneDay_RT100_500','.svg']))
close(fig1)



data = [Predict_task_All,Dynamic_task_All,Predict_reactive_All];
% [r, p] = partialcorr(Predict_task_All,Dynamic_task_All,Predict_reactive_All);
% % 3. 输出结果
% fprintf('偏相关系数 r = %.3f\n', r);
% fprintf('显著性 p值 = %.3f\n', p);

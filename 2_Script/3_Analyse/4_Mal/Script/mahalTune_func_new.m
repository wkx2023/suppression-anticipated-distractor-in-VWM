function  [cos_amp, d_tune_plot] = mahalTune_func_new(data,data_test,theta,theta_test,angspace,bin_width)
...
d_tune_plot = nan(size(data,1),length(angspace),size(data,3));
...
for trl=1:size(data,1)
    ...
    for ti=1:size(data,3)
        ...
        if ~isnan(trn_dat(:,:,ti))
            ...
            d_tune_plot(trl,:,ti) = -(cos(angspace).*squeeze(d_tune(trl,:,ti))');

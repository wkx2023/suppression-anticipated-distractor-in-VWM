...
% for decoding accuracy (see Sprague, T. C., Ester, E. F. & Serences, J. T. Restoring Latent Visual Working Memory Representations in Human Cortex. Neuron 91, 694?07 (2016))
...
d_tune=nan(size(data,1),length(angspace),size(data,3),size(data,3));
cos_amp=nan(size(data,1),size(data,3),size(data,3));
...
for trl=1:size(data,1)
    ...
    for tj = 1:size(data,3)
        for ti=1:size(data,3)
            if ~isnan(trn_dat(:,:,tj))
                % the covariance matrix is computed for each time-point and excluding the test-trial
                sigma = covdiag(trn_dat(:,:,tj)); % here covariance is estimated using shrinkage (see below from Ledoit & Wolf, 2004).
                % calculates the distances between the trial and all angle bins
                d_tune(trl,:,tj,ti) = pdist2(squeeze(m(:,:,tj)), squeeze(data(trl,:,ti)),'mahalanobis',sigma);
                ...
                cos_amp(trl,tj,ti)=-(mean(cos(angspace).*squeeze(d_tune(trl,:,tj,ti))'));
            end
        end
    end
end

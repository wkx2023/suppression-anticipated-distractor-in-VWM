function [clusters, p_values, t_sums, permutation_distribution ] = permutest_v2( trial_group_1, trial_group_2, dependent_samples, ...
...
if dependent_samples
    ...
    if num_data_points_y == 1 
        ...
        A = trial_group_1;   % 250x250x20
        B = trial_group_2;   % 250x250x1000
        
        n1 = size(A,3);
        n2 = size(B,3);
        
        mu1 = sum(A,3) / n1;            % 250x250
        mu2 = sum(B,3) / n2;            % 250x250
        
        S1  = sum(conj(A).*A, 3);        % sum of squares, 250x250
        S2  = sum(conj(B).*B, 3);        % 250x250
        
        var1 = (S1 - n1 * conj(mu1).*mu1) / (n1 - 1);   % sample variance
        var2 = (S2 - n2 * conj(mu2).*mu2) / (n2 - 1);
        
        std1 = sqrt(var1);
        std2 = sqrt(var2);
        
        den  = sqrt(std1.^2/n1 + std2.^2/n2);
        t_value_vector = (mu1 - mu2) ./ den;            % 250x250

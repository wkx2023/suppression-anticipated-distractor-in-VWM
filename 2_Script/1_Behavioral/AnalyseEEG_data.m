...
%  folder是原始数据存储的位置
%  participant 是被试的序号
function AnalyseEEG_data(folder,participant,sube)
rootDir = pwd;
outputDir = [rootDir,'\EEG_Behavioral_1022\'];
...
DelMat  = strcat('F:\Behavioral_Data\DelTrial\Task\','Del',num2str(sube),'.mat');
load(DelMat);
[~, loc] = ismember(del, std_trial_num);
...
res_time(loc) =NaN;
key_time(loc) = NaN;
index_type_all(loc) = NaN;
dis(loc) = NaN;
absError(loc) = NaN;
Degree(loc) = NaN;

cue = cue(~isnan(cue));
noMemory = noMemory(~isnan(noMemory));
new_error = new_error(~isnan(new_error));
res = res(~isnan(res));
res_time = res_time(~isnan(res_time));
key_time = key_time(~isnan(key_time));
index_type_all = index_type_all(~isnan(index_type_all));
dis = dis(~isnan(dis));
absError = absError(~isnan(absError));
Degree = Degree(~isnan(Degree));
